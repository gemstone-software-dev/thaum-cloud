...
Example assets for the **[Azure Container Apps + GitHub Actions guide](../../doc/quickstart/cloud/azure/README.md)**.

| File | Purpose |
|------|---------|
| [Dockerfile.example](Dockerfile.example) | Pin upstream Thaum image; `COPY` `thaum.toml` → `/etc/thaum/thaum.toml` |
| [deploy.yml.example](deploy.yml.example) | **Non-default:** build → schema-check → push **ACR** → update Container App ([GHCR path](../../doc/quickstart/cloud/azure/README.md) uses repo root `.github/workflows/deploy-aca.yml`) |
